(function(){
    ...
    Scroll.prototype = {
        ...
        momentumY : 0, lastTouchTime : 0, oldY : 0,
        ...
        removePage : function (el) {
            ...
            var that = this;
            setTimeout(function (){
                if (that.currentPage < targetIndex) {
                    // we removed page to the right, it's fine
                } else if (that.currentPage > targetIndex) {
                    // we removed page to the left, must probably scroll some
                    that.gotoPage(that.currentPage--);
                } else {
                    console.log(that.currentPage, that.pages.length)
                    if (that.currentPage >= that.pages.length ) {
                        // we removed last page, scroll back
                        that.gotoPage(that.pages.length - 1)
                    } else {
                        // we removed current page, but it wasn't last : try to update
                        that.gotoPage(that.currentPage)
                    }
                }
            }, 10);
        ...
        touchStart : function (e) {
            ...
            var time = Date.now() - this.momentum.frameTime;
            if (time < 1000) {
                console.log('STILL SCROLLING!')
                // we're still scrolling: fuck
                var matrix = getComputedStyle(this.scroller, null)[vendor + 'Transform'].replace(/[^0-9-.,]/g, '').split(','),
                    actualY = matrix[5] * 1 || 0;
                
                this.move(this.x, actualY)
            }
           
            this.scroller.style.webkitTransitionDuration = '0.01ms'
            this.scroller.style.webkitTransitionDelay = '-1ms'
        ...
        scrollMomentum : function (e) {
            ...
            console.log(this.y + 'vs' + (this.y + this.momentum.vy) + ' vs ' + this.prevY + '/' + touch.pageY)

            this.scroller.style['-webkit-transition'] = '-webkit-transform 1.4s ease-out';
            this.move(this.x, this.y + this.momentum.vy)

            var that = this;
            this.frame = setTimeout(function (){
                if (that.y > 0 || that.y < that.height) {
                    //this.scroller.style['-webkit-transition'] = '-webkit-transform 1s ease-out';
                    that.move(that.x, that.y)
                    that.momentum.frameTime = Date.now();
                    if (newY > 0)
                        that.resetXY(0, -newY)
                    else if (newY < that.height)
                        that.resetXY(0, that.height - that.y)
                }
        ...
        render : function () {
            ...
            console.log(t + ':' + dx + ':' + dy + '::' + this.animation.ox + ':' + this.animation.oy + ' [' + this.x + ',' + this.y + ']');
